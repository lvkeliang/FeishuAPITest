...
class TestCaseLoader:
    ...
    def __init__(self, base_path: str = "tests/test_data"):
        ...
        self._init_file_keys()  # 初始化时自动上传 sample.xxx 文件

    def _init_file_keys(self):
        """自动上传 sample.xxx 文件并保存 file_key 映射"""
        folder = "tests/test_data/message/file"
        url = "https://open.feishu.cn/open-apis/im/v1/files"
        token = config.env_config["feishu"].get("tenant_access_token")
        headers = {"Authorization": f"Bearer {token}"}
        file_type_map = {
            "mp4": "mp4", "pdf": "pdf", "doc": "doc", "xls": "xls",
            "ppt": "ppt", "opus": "opus", "stream": "stream"
        }
        file_keys = {}
        for fname in os.listdir(folder):
            ...
            if ext not in file_type_map:
                continue
            files = {
                "file_type": (None, file_type_map[ext]),
                "file_name": (None, fname),
                "file": (fname, open(fpath, "rb"))
            }
            try:
                resp = requests.post(url, headers=headers, files=files)
                data = resp.json()
                if "data" in data and "file_key" in data["data"]:
                    file_keys[fname] = data["data"]["file_key"]
                    print(f"{fname} 上传成功，file_key: {data['data']['file_key']}")
                else:
                    print(f"{fname} 上传失败，返回: {data}")
            except Exception as e:
                print(f"{fname} 上传异常: {e}")
        self._context["file_keys"] = file_keys
    ...
    def execute_setup(self, setup_actions: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for action in setup_actions:
            ...
            if action_type == "get_tenant_access_token":
                ...
                if file_name in self._context.get("file_keys", {}):
                    context[action["save_to"]] = self._context["file_keys"][file_name]
                else:
                    # 如果没有自动上传的 file_key，尝试手动上传
                    try:
                        file_key = self._upload_file_manual(file_path, file_type)
                        context[action["save_to"]] = file_key
                    except Exception as e:
                        print(f"文件上传失败 {file_path}: {str(e)}")
                        context[action["save_to"]] = None
    ...
    def _replace_explicit_variable(self, var_name: str, context: Dict[str, Any]) -> Any:
        """处理显式变量引用：$var$ 或 $response.data.field$ 或 $sample.mp4$"""
        if var_name.startswith("response."):
            ...
            return value
        # 新增：如果变量名是 sample.xxx，自动查找 file_keys
        if var_name in self._context.get("file_keys", {}):
            return self._context["file_keys"][var_name]
        return self._context.get(var_name, f"${var_name}$")  # 保持原格式若未找到

    def _upload_file_manual(self, file_path: str, file_type: str) -> str:
        """手动上传文件并返回 file_key"""
        url = "https://open.feishu.cn/open-apis/im/v1/files"
        token = config.env_config["feishu"].get("tenant_access_token")
        headers = {"Authorization": f"Bearer {token}"}
        ...
        file_name = os.path.basename(file_path)
        feishu_file_type = file_type_map.get(file_type, file_type)
